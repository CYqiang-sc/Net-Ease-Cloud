		...
		<view class="wrapper">
			...
			<view v-if="!isPass" class="input-content">
				...
				<view class="input-item">
					...
					<input type="password" placeholder="请输入密码" id="password" v-model="password" @keyup.enter="login" />
				</view>
			</view>
			<view v-else class="input-content">
				...
				<view class="input-item-code">
					...
					<view class="code-btn">
						<button :disabled="coderTimer" :class="{ active: !coderTimer }" @click="codeBtn">
							{{ title }}
						</button>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			login() {
				...
				// 调网易云登录
				login(this.phone, this.password, res => {
				...
				uni.request({
					url: "http://162.14.109.33:8081/login/bind",
					data: {
						appId: openid,
						phone: this.phone,
