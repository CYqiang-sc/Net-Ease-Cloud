<template>
	<view class="content">
		...
		<!-- 歌曲列表 -->
		<view class="songList">
			<view class="songList-item" v-for="(item,index) in songDetailList" :key="item.song.id">
				<navigator @click="addCurrentSong(item.song.id)" :url="'/pages/song/song?id='+item.song.id+'&name='+item.song.name"
					hover-class="detail-hover">
					<!-- <view class="num">{{index+1}}</view> -->
					<image class="avatar" :src="item.song.al.picUrl" lazy-load></image>
					<view class="song-detail">
						<view class="song-name">{{item.song.name}}</view>
						<view class="song-title">
							<text class="song-detail-vip" v-if="item.song.fee==1">VIP</text>
							<text class="song-detail-sq">SQ</text>
							<!-- 第一个歌手名 -->
							<view class="song-singer"> {{item.song.ar[0].name}}</view>
						</view>
					</view>
				</navigator>
				<image src="/static/images/mv.png" class="mv" mode="widthFix" v-if="item.song.mv != 0"
					@click="gotoPlayMV(item.song.mv,item)"></image>
				<image src="/static/images/more.png" class="more" mode="widthFix" @click="toShowAction(item.song.id)">
				</image>
			</view>
		</view>
		<view>
		...
		<view>
			...
			<view :class="['more-action',isShowCollect==false ? 'more-action-hide':'more-action-show']">
				...
				<view class="collectlist-item">
					...
					<view class="action-item" v-for="(item,index) in collectList" :key="index">
						<!-- <view v-if="item.creator.userId==uid"> -->
			...
			<view :class="['more-action',isshowplaylist==false ? 'more-action-hide':'more-action-show']">
				...
				<input class="uni-input" maxlength="20" ref="input1" adjust-position="true" @input="onKeyInput" placeholder="请输入歌单名称" />
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			//获取用户喜欢的列表
			getUserRecord(this.uid, res => {
				console.log("播放", res)
				if (res.code == 200) {
					this.songDetailList = res.allData
					console.log("播放记录",this.songDetailList)
					console.log("播放记录长度",this.songDetailList.length)
				}
			})
		},
		methods: {
			...
			toShowAction(id) {
				...
				getSongDetail(id, res => {
					...
					if (res.code == 200) {
						...
						console.log("单曲", this.singleSong)
					}
				})
			},
			toHideAction() {
